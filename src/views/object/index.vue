            ...
            <Column v-for="col of selectedColumns" :field="col.data" :header="col.label" :key="col.data"
              ...
              <template #body="slotProps">
                <template v-if="col.data == 'id'">
                  <RouterLink :to="'/Object/edit/' + slotProps.data[col.data]">
...
import PopupAdd from "../../components/Object/PopupAdd.vue";
...
const { model, headerForm, visibleDialog } = storeToRefs(store_Object);
const openNew = () => {
  model.value = {};
...
const showing = ref([]);
const column_cache = "columns_Object"; ////
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
  loading.value = true;
  ObjectApi.table(lazyParams.value).then((res) => {
    // console.log(res);
